...
def parse_file(filepath, filename):
    ...
    for linenum, line in enumerate(file.readlines()):
        ...
        if is_find_change == True:
            if ' | [Alfred Jiang](https://yujiuqie.github.io) | - |' in line:
                data_string = line.split(' | ')[1]
                break

        if '| 序号 | 录入时间 | 录入人 | 备注 |' in line:
            is_find_change = True
    ...
    if len(data_string) > 0:
        ...
        replace_string = "date: 2020-08-08"
        file_new = open(fullpath, "r")
        file_content = file_new.read()
        result = file_content.replace(replace_string,"data: "+ data_string)
